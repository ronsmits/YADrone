...
public class XugglerDecoder implements VideoDecoder {
    private static final Logger LOG = Logger.getLogger(XugglerDecoder.class);

    private ImageListener listener;
    private boolean doStop = false;

    @Override
    public void decode(final InputStream is) {
        ...
        while (!doStop && container.readNextPacket(packet) >= 0) {
            ...
            if (packet.getStreamIndex() == videoStreamId) {
                 ...
                 */
                final IVideoPicture picture = IVideoPicture.make(videoCoder.getPixelType(), videoCoder.getWidth(),
                ...
                try {
                    ...
                    while (offset < packet.getSize()) {
                        ...
                        /*
                         * Some decoders will consume data in a packet, but will not be able to construct a full video
                         * picture yet. Therefore you should always check if you got a complete picture from the decoder
                         */
                        if (picture.isComplete()) {
                            // System.out.println("1 Picture complete");
                            IVideoPicture newPic = picture;
                            /*
                             * If the resampler is not null, that means we didn't get the video in BGR24 format and need
                            ...
                            /**
                             * We could just display the images as quickly as we decode them, but it turns out we can
                             * decode a lot faster than you think. So instead, the following code does a poor-man's
                             * version of trying to match up the frame-rate requested for each IVideoPicture with the
                             * system clock time on your computer. Remember that all Xuggler IAudioSamples and
                             * IVideoPicture objects always give timestamps in Microseconds, relative to the first
                             * decoded item. If instead you used the packet timestamps, they can be in different units
                            ...
                            if (firstTimestampInStream == Global.NO_PTS) {
                                ...
                                if (millisecondsToSleep > 0) {
                                    ...
                                    // {
                                    // // we might get this when the user closes the dialog box, so just return from the
                                    // method.
                                    // return;
                                    // }
                                }
        ...
        LOG.trace("XugglerDecoder: clean up and close stream ...");
        /*
         * Technically since we're exiting anyway, these will be cleaned up by the garbage collector... but because
         * we're nice people and want to be invited places for Christmas, we're going to show how to clean up.
         */
        if (videoCoder != null) {
            videoCoder.close();
            videoCoder = null;
    ...
    @Override
    public void setImageListener(final ImageListener listener) {
